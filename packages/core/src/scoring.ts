 ...
 */
export function computeRI(ep: EndpointRaw, weights: VulnWeights = DEFAULT_VULN_WEIGHTS): number {
  const se = ep.s * ep.e;
...
export function getRIBand(ri: number): RIBand {
  ...
  if (ri >= 1.0) return 'High';
  if (ri >= 0.4) return 'Medium';
  return 'Low';
}
 ...
 */
export function getRIColor(ri: number): string {
 ...
 */
export function computeRIBreakdown(
  ep: Endpoint,
  weights: VulnWeights = DEFAULT_VULN_WEIGHTS
): RIBreakdown {
  const v = computeV(ep, weights);
 ...
 */
export function predictedZombieDate(ep: EndpointRaw): string | null {
  const state = computeState(ep);
  if (state === 'zombie' || ep.decayProb < 0.5) return null;
