...
import { computeV, getVulnBreakdown } from "./vulnerability";
...
function buildEndpoint(raw, weights = defaultWeights) {
...
describe("Vulnerability Functions", () => {
    describe("computeV", () => {
        ...
        it("handles basic auth as vulnerable", () => {
            const basicAuth = { ...secureEndpoint, auth: "basic" };
            const v = computeV(basicAuth, defaultWeights);
            expect(v).toBe(0.35);
        });
        it("handles api_key auth as vulnerable", () => {
            const apiKey = { ...secureEndpoint, auth: "api_key" };
            const v = computeV(apiKey, defaultWeights);
            expect(v).toBe(0.35);
        });
        it("handles jwt and oauth2 as secure", () => {
            const jwt = { ...secureEndpoint, auth: "jwt" };
            const oauth = { ...secureEndpoint, auth: "oauth2" };
