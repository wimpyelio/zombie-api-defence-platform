...
const riRoutes: FastifyPluginAsyncZod = async (app) => {
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      // Use serializer - toEndpointFull returns Endpoint for computeRIBreakdown
      const endpoint = toEndpointFull(ep as unknown as PrismaEndpoint);
  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      for (const ep of endpoints) {
        // Use serializer - toEndpointFull returns Endpoint for all computations
        const endpoint = toEndpointFull(ep as unknown as PrismaEndpoint);
        const v = computeV(endpoint);
        const ri = computeRI(endpoint);
        const band = getRIBand(ri);
        ...
        byBand[band.toUpperCase() as keyof typeof byBand] =
          (byBand[band.toUpperCase() as keyof typeof byBand] || 0) + 1;
        byState[state.toUpperCase() as keyof typeof byState] =
          (byState[state.toUpperCase() as keyof typeof byState] || 0) + 1;
